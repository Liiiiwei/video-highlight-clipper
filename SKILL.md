...
2. 詢問使用者每個片段的裁切方式（因為畫面中可能有多人）：
   - **left** — 聚焦畫面左側的人
   - **right** — 聚焦畫面右側的人
   - **center** — 聚焦畫面中央
   - **不裁切** — 保持原始 16:9 畫面

   如果使用者說「全部都裁左邊」或類似指示，則統一套用，不需逐段詢問。

3. 對每個選定片段，依序執行：

#### 4.1 剪輯影片片段（含 9:16 裁切）
```bash
python3 scripts/clip_video.py <video_path> <start_time> <end_time> <output_path> --reencode --crop <left|right|center>
```
- 使用 re-encode 模式確保精確切割
- `--crop` 參數裁切為 9:16 直式（聚焦指定位置的人）
- 如果使用者選擇不裁切，則省略 `--crop` 參數
- 輸出: `clip.mp4`
...
### 9:16 直式裁切
使用 `--crop left|right|center` 將 16:9 橫式畫面裁切為 9:16 直式，適合 Reels/Shorts。
對於兩人並排的直播畫面，`left` 聚焦左側人物，`right` 聚焦右側人物。
裁切會自動啟用 re-encode 模式。
